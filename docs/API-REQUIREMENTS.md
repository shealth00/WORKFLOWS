...
## Patient directory (static import)

- Staff route **`/patient-directory`** loads **`/patient-directory/profiles.json`** (generated by **`npm run generate:patient-profiles`** from **`patient upload/*.xlsx`**). See [PATIENT-DIRECTORY.md](./PATIENT-DIRECTORY.md). Demo fallback: **`profiles.demo.json`**.

## Related docs

- [FIREBASE-AUTH-SETUP.md](./FIREBASE-AUTH-SETUP.md) — Google sign-in and authorized domains
- [PATIENT-DIRECTORY.md](./PATIENT-DIRECTORY.md) — spreadsheet import and Hosting assets
